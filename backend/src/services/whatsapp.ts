...
export class WhatsAppService {
  ...
  private formatPhoneNumber(phone: string): string {
    ...
    // If number already starts with '201', return as is
    if (formatted.startsWith('201')) {
      return formatted;
    }
    
    // If number starts with just '20', add '1'
    if (formatted.startsWith('20')) {
      return `201${formatted.substring(2)}`;
    }
    
    // Otherwise add full prefix
    return `201${formatted}`;
  }
