...
import { XMarkIcon, DocumentTextIcon, CheckIcon, ChevronLeftIcon, ChevronRightIcon, ArrowPathIcon } from '@heroicons/react/24/outline';
import toast from 'react-hot-toast';
import { financialService } from '../../services/financialService';

interface BulkShippingCostRevertDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onRevert: (orderIds: number[]) => Promise<void>;
}
...
export default function BulkShippingCostRevertDialog({
  isOpen,
  onClose,
  onRevert,
}: BulkShippingCostRevertDialogProps) {
  const [pastedText, setPastedText] = useState('');
  const [isProcessing, setIsProcessing] = useState(false);
  const [parsedOrderIds, setParsedOrderIds] = useState<number[]>([]);
  ...
  const parseText = async () => {
    if (!pastedText.trim()) {
      toast.error('Please paste the order IDs');
      return;
    }
    ...
    for (const line of lines) {
      ...
      if (!matched) {
        ...
        if (numbers) {
          const orderId = parseInt(numbers[0], 10);
          if (!isNaN(orderId) && orderId > 0) {
            orderIds.push(orderId);
          }
        }
    ...
    if (orderIds.length === 0) {
      toast.error('No valid order IDs found. Please check the format.');
      return;
    }

    // Remove duplicates from parsed IDs
    const uniqueOrderIds = [...new Set(orderIds)];
    setParsedOrderIds(uniqueOrderIds);

    // Check for duplicates (multiple orders with same order number)
    setIsCheckingDuplicates(true);
    try {
      const duplicatesFound = await checkForDuplicates(uniqueOrderIds);
      ...
      if (duplicatesFound.length > 0) {
        ...
        toast.success(`Parsed ${uniqueOrderIds.length} order IDs. Found ${duplicatesFound.length} duplicate(s) that need selection.`);
      } else {
        setIsCheckingDuplicates(false);
        toast.success(`Parsed ${uniqueOrderIds.length} unique order IDs. No duplicates found.`);
      }
    } catch (error: any) {
      setIsCheckingDuplicates(false);
  ...
  const checkForDuplicates = async (orderIds: number[]): Promise<DuplicateOrder[]> => {
    try {
      ...
      // Find orders that match the provided order IDs
      const matchedOrders = orderIds
        .map(orderId => allOrders.find(o => o.id === orderId))
        .filter((order): order is Order => order !== undefined);
      
      // For each matched order, check if there are multiple orders with the same order number
      // (same logic as import dialog)
      const processedOrderNumbers = new Set<string>();
      
      for (const order of matchedOrders) {
        const orderNum = order.name.replace(/[^0-9]/g, '');
        ...
        // Find ALL orders with this order number (not just the ones in our list)
        const matchingOrders = allOrders.filter(o => {
          const oOrderNum = o.name.replace(/[^0-9]/g, '');
          return oOrderNum === orderNum;
        });
        
        // If there are multiple orders with this order number, it's a duplicate
  ...
  const handleRevert = async () => {
    if (parsedOrderIds.length === 0) {
      toast.error('Please parse the order IDs first');
      return;
    }
    ...
    // Build final list of order IDs to revert
    let finalOrderIds = [...parsedOrderIds];
    
    // Remove duplicate order IDs that weren't selected, add selected ones
    if (duplicates.length > 0) {
      // Remove all orders that are duplicates
      duplicates.forEach(dup => {
        dup.orders.forEach(order => {
          const index = finalOrderIds.indexOf(order.id);
          if (index > -1) {
            finalOrderIds.splice(index, 1);
          }
        });
      });
      
      // Add only the selected orders
      duplicates.forEach(dup => {
        if (dup.selectedOrderId) {
          finalOrderIds.push(dup.selectedOrderId);
        }
      });
    }
    ...
    try {
      await onRevert(finalOrderIds);
      toast.success(`Successfully reverted shipping costs for ${finalOrderIds.length} orders`);
      handleClose();
    } catch (error: any) {
      toast.error(error.message || 'Failed to revert shipping costs');
    } finally {
  ...
  const handleConfirmDuplicates = () => {
    ...
    // Close duplicate dialog - user can now click Revert
    setShowDuplicateDialog(false);
    toast.success('All duplicate orders have been resolved. Click "Revert" to proceed.');
  };
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:p-6">
                ...
                <div className="sm:flex sm:items-start">
                  <div className="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-red-100 sm:mx-0 sm:h-10 sm:w-10">
                    <ArrowPathIcon className="h-6 w-6 text-red-600" aria-hidden="true" />
                  </div>
                  <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                    <Dialog.Title as="h3" className="text-base font-semibold leading-6 text-gray-900">
                      Bulk Revert Shipping Costs
                    </Dialog.Title>
                    <div className="mt-4 space-y-4">
                      <div>
                        <label htmlFor="pasted-order-ids" className="block text-sm font-medium text-gray-700 mb-1">
                          Paste Order IDs
                        </label>
                        <p className="text-xs text-gray-500 mb-2">
                          Format: Order IDs or order numbers (e.g., 12345, #1120, Order #1120, ID: 12345)
                        </p>
                        <textarea
                          id="pasted-order-ids"
                          rows={8}
                      ...
                      <div>
                        ...
                        >
                          {isCheckingDuplicates ? 'Checking for duplicates...' : 'Parse Order IDs'}
                        </button>
                      </div>
                        ...
                        <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                          <div className="flex justify-between items-center mb-3">
                            <h4 className="text-sm font-medium text-gray-900">
                              Parsed Order IDs ({parsedOrderIds.length})
                            </h4>
                          </div>
                          <div className="max-h-48 overflow-y-auto">
                            <div className="grid grid-cols-2 gap-2 text-xs">
                              {parsedOrderIds.slice(0, 20).map((orderId, index) => (
                                <div key={index} className="font-mono">
                                  {orderId}
                                </div>
                              ))}
                              {parsedOrderIds.length > 20 && (
                                <div className="col-span-2 text-gray-500 text-center">
                                  ... and {parsedOrderIds.length - 20} more
                                </div>
                              )}
                            </div>
                          </div>
                ...
                <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                  <button
                    ...
                    disabled={parsedOrderIds.length === 0 || isProcessing || isCheckingDuplicates || (duplicates.length > 0 && duplicates.some(dup => !dup.selectedOrderId))}
                    className="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:ml-3 sm:w-auto disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isProcessing ? 'Reverting...' : `Revert ${parsedOrderIds.length} Orders`}
                  </button>
                  {duplicates.length > 0 && (
                    <button
                      type="button"
                                ...
                                <div className="border border-gray-200 rounded-lg p-4">
                                  <div className="flex items-center justify-between mb-4">
                                    <h4 className="text-lg font-semibold text-gray-900">
                                      {currentDuplicate.orderNumber}
                                    </h4>
                                  </div>
                                  <p className="text-sm text-gray-600 mb-4">
                                      Multiple orders found with this number. Please select the correct one to revert:
                                    </p>
                                            ...
                                            <div className="flex-1">
                                              ...
                                              <div className="text-sm text-gray-600 space-y-1">
                                                ...
                                                <div className="text-xs text-gray-500 pt-1">
                                                  Created: {new Date(order.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                                </div>
                                              </div>
                                            </div>
                                          </div>
